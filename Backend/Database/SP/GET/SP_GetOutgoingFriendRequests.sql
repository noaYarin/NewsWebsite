CREATE PROCEDURE SP_GetOutgoingFriendRequests
    @UserId INT
AS
BEGIN
    -- Get outgoing friend requests for a user (requests sent BY the user)
    SELECT 
        CASE 
    ...
    WHERE (f.UserOneId = @UserId OR f.UserTwoId = @UserId)
      ...
      AND f.ActionUserId = @UserId  -- Requests sent BY the user
