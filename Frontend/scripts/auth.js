...
function validateEmail(email) {
  ...
  if (!CONFIG.VALIDATION_REGEX.EMAIL.test(email)) return { valid: false, message: "Please enter a valid email address" };
  return { valid: true };
}
...
function validatePassword(password) {
  const hasLetterAndNumber = CONFIG.VALIDATION_REGEX.PASSWORD_LETTER_AND_NUMBER.test(password) || CONFIG.VALIDATION_REGEX.PASSWORD_LETTER_AND_SPECIAL.test(password);
  if (!password) return { valid: false, message: "Password is required" };
  if (password.length < CONFIG.PASSWORD_REQUIREMENTS.MIN_LENGTH)
